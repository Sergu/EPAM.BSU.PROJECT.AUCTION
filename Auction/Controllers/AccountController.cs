    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    WebSecurity.CreateUserAndAccount(model.login, model.Password, new { login=model.login,email=model.email,money = 0});
        ...
        public ActionResult ExternalLoginConfirmation(RegisterExternalLoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    User user = db.User.FirstOrDefault(u => u.login.ToLower() == model.UserName.ToLower());
                    // Проверка наличия пользователя в базе данных
                    if (user == null)
                    {
                        // Добавление имени в таблицу профиля
                        db.User.Add(new User { login = model.UserName });
                        db.SaveChanges();
